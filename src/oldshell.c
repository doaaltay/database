...
struct Config history_config()
{
  // reads the history of commands
  ...
  if (file == NULL)
  {
    ...
    if (logfile != NULL)
    {
      fprintf(logfile, "Error opening the history file: %s", timestamp);
    }

    exit(EXIT_FAILURE);
  }

  fprintf(file, "History of commands\n");

  for (int i = 0; i < config.history_counter; i++)
  {
    fprintf(file, "%s\n", config.history[i]);
  }
...
struct Config prompt_config()
{
  /**
   * this config file lets an user enter a custom prompt
   * if no custom prompt is entered, the default prompt is "> "
   */
  // struct Config config;
  FILE *prompt_config = fopen(CUSTOMPROMPT_CONFIG, "r");
  if (prompt_config == NULL)
  {
    perror("Error opening the custom prompt config file");
    time_t now = time(NULL);
    char *timestamp = ctime(&now);
    if (logfile != NULL)
    {
      fprintf(logfile, "Error opening the custom prompt config file : %s", timestamp);
    }

    exit(EXIT_FAILURE);
  }
  char line[BUFSIZE];
  while (fgets(line, sizeof(line), prompt_config))
  {
    if (line[0] == '#')
    {
      continue;
  ...
  if (strncmp(line, "enter custom prompt:", 20) == 0)
  {
    strncpy(config.customprompt, line + 20, sizeof(config.customprompt) - 1);
    config.customprompt[sizeof(config.customprompt) - 1] = '\0';
    strcpy(COMMANDPROMPT, config.customprompt);
  }
...
struct Config login_config()
{
  ...
  if (login_config == NULL)
  {
    ...
    if (logfile != NULL)
    {
      fprintf(logfile, "Error opening the login config file : %s", timestamp);
    }
  ...
  if (strncmp(line, "enter username:", 15) == 0)
  {
    ...
    config.username[sizeof(config.username) - 1] = '\0'; // null terminate
  }
...
int main(int argc, char **argv)
{
  ...
  fprintf(logfile, ".LOG: %s", timestamp);
...
int execute_command(char **args) // execute command
{
  ...
  for (i = 0; i < get_builtins_number(); i++) // looping over the commands,  execute the one that matches
  {
    if (strcmp(args[0], builtin_commands[i]) == 0)
    {
      ...
      if (config.history_counter < MAX_HISTORY)
      {
        strncpy(config.history[config.history_counter], args[0], MAX_COMMAND_LENGTH - 1);
        config.history[config.history_counter][MAX_COMMAND_LENGTH - 1] = '\0';
        config.history_counter++;
      }
      ...
      if (logfile != NULL)
      {
        fprintf(logfile, "Executing command %s at : %s \n", args[0], timestamp);
      }
...
void command_loop(void)
{
  ...
  while (status)
  {
    printf(COMMANDPROMPT);          // add config file for a custom command
...
char **split_line(char *line)
{
  ...
  if (!tokens) // if memory allocation fails
  {
    ...
    if (logfile != NULL)
    {
      fprintf(logfile, "Memory allocation error: %s\n", timestamp);
    }
...
int cd(char **args)
{
  ...
  else
  {
    if (chdir(args[1]) != 0) // change the directory to the requested one
    {
      ...
      if (logfile != NULL)
      {
        fprintf(logfile, "cd failed: %s", timestamp);
      }
...
int help_command(char **args)
{
  ...
  printf("This is a simple shell program. Enter commands\n");
